public class Instructions extends lab3 {
    public Instructions(int instructionsExecuted) {

        String elem = cleanedFile.get(LineCount).get(0);
        for (int i = 0; i < instructionsExecuted; i++) {
            if (cleanedFile.get(LineCount).get(0).contains(":")) {
                new LabelCase();
                continue;
            } else if (elem.equals("add")) { // add
                ...
                if (registers.get(cleanedFile.get(LineCount).get(1)).equals(registers.get(cleanedFile.get(LineCount).get(2)))) {
                    LineCount = LabelLocation.get(cleanedFile.get(LineCount).get(3));  // lineCount = lineCount - LabelLocation
                }
                // GETTING STUCK HERE
